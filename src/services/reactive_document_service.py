"""Reactive document service — CRUD for uploaded documents with MinIO S3 storage."""
...
class ReactiveDocumentService:
    """Service for reactive documents (isolated from chat documents)."""
    ...
    def __init__(
        ...
        self.repo = ReactiveDocumentRepository(session)
    ...
    def _get_knowledge_service(self):
        if self._knowledge_service is None:
            ...
            self._knowledge_service = ReactiveKnowledgeService(self.session)
    ...
    async def create_document(
        ...
        """Create a reactive document record with its S3 key already set."""
        kb = await self._get_knowledge_service().get_knowledge_base(kb_id, user_id)

        doc = ReactiveDocument(
            reactive_knowledge_base_id=kb.id,
            file_id=s3_key,
            filename=filename,
    ...
    async def upload_document(
        ...
        if self._storage is None:
            raise RuntimeError("ReactiveDocumentService requires a StoragePort to perform uploads")
        ...
        s3_key = f"reactive_kb/{kb_id}/{file_id}{ext}"
    ...
    ) -> list[ReactiveDocument]:
        """List reactive documents, optionally filtered by knowledge base."""
        if knowledge_base_id is not None:
            return await self.repo.list_by_knowledge_base(knowledge_base_id)
        return await self.repo.list()

    async def delete_document(self, doc_id: int) -> None:
        """Delete reactive document from DB, MinIO, and Qdrant.

        Order: DB commit first, then external systems.
        """
        doc = await self.repo.get_by_id(doc_id)
        if not doc:
            raise NotFoundError(f"Reactive document {doc_id} not found")

        kb_id = doc.reactive_knowledge_base_id
        file_id = doc.file_id

        # 1. Delete from Relational DB first (source of truth)
        await self.repo.delete(doc)
        await self.session.commit()
        logger.info("Deleted reactive document %d from DB", doc_id)

        # 2. Delete from Qdrant vector database (best-effort)
        from src.persistencia.vector import VectorRepository
        vector_repo = VectorRepository()
        try:
            await vector_repo.delete_by_doc_id(kb_id, doc_id, prefix="reactive_kb_")
            logger.info("Deleted vectors for reactive document %d from Qdrant", doc_id)
        except Exception:
            logger.exception("Failed to delete vectors for reactive document %d", doc_id)
