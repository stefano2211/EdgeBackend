"""RAG tool for DeepAgents — multi-stage retrieval from Qdrant.

Uses the composable RetrievalPipeline (2026 best practices):
  enhance → embed (dense+sparse) → hybrid search (RRF) → rerank → format
...
async def _rag_retrieve_impl(
    ...
    """Internal implementation using the composable RetrievalPipeline.

    Pipeline stages:
    1. Query Enhancement (multi-query + HyDE)
    2. Dense + Sparse embedding
    3. Hybrid search with RRF fusion in Qdrant
...
def create_rag_tool(knowledge_base_id: str) -> StructuredTool:
    ...
    async def _bound_rag_retrieve(query: str, top_k: int = 5) -> str:
        ...
        return await _rag_retrieve_impl(knowledge_base_id, query, top_k)
