...
def _build_reactive_industrial_subagent(
    ...
    if knowledge_base_id:
        tools.append(create_reactive_rag_tool(knowledge_base_id))
...
def _build_reactive_historical_subagent(knowledge_base_id: str | None = None) -> dict:
...
def _build_reactive_vl_subagent(knowledge_base_id: str | None = None) -> dict:
...
def _build_reactive_s1_coordinator(knowledge_base_id: str | None = None) -> dict:
    ...
    return {
        ...
        "model": get_chat_model(),
        "subagents": [
            _build_reactive_historical_subagent(knowledge_base_id),
            _build_reactive_vl_subagent(knowledge_base_id),
        ],
    }
...
REACTIVE_SUBAGENT_BUILDERS: dict[str, Callable[[str | None], dict]] = {
...
def get_reactive_subagents(
    ...
    Args:
        ...
        knowledge_base_id: Optional ID to bind to reactive sub-agent tools.
        enable_mcp: Whether to enable reactive MCP tools on the industrial sub-agent.
    """
    ...
    for name in names:
        if name not in REACTIVE_SUBAGENT_BUILDERS:
            continue
        if name == "industrial":
            result.append(REACTIVE_SUBAGENT_BUILDERS[name](knowledge_base_id, enable_mcp=enable_mcp))
        else:
            result.append(REACTIVE_SUBAGENT_BUILDERS[name](knowledge_base_id))
    return result
...
def get_reactive_subagent_descriptions(
    ...
    for name in names:
        if name not in REACTIVE_SUBAGENT_BUILDERS:
            continue
        cfg = REACTIVE_SUBAGENT_BUILDERS[name](knowledge_base_id)
