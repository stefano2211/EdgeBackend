            ...
            >
                <div class="flex items-start justify-between mb-3">
                    ...
                    <span
                        ...
                        :class="conn.status === 'connected' ? 'bg-emerald-400' : conn.status === 'error' ? 'bg-red-400' : 'bg-yellow-400'"
                    />
                </div>

                <div class="flex items-center gap-2 mb-3 text-xs text-[#7a7a7a]">
                    <span v-if="conn.available_in_chat" class="px-2 py-0.5 bg-white/[0.06] rounded">Chat</span>
                    <span v-if="conn.available_in_reactive" class="px-2 py-0.5 bg-white/[0.06] rounded">Reactiva</span>
                    <span class="px-2 py-0.5 bg-white/[0.06] rounded">{{ getTableCount(conn) }} tablas</span>
                </div>

                <div class="flex items-center gap-2">
                    <button @click="testConnection(conn)" class="text-xs px-2 py-1 bg-white/[0.06] rounded hover:bg-white/[0.1]">Test</button>
                    <button @click="discoverSchema(conn)" class="text-xs px-2 py-1 bg-white/[0.06] rounded hover:bg-white/[0.1]">Schema</button>
                    <button @click="openEdit(conn)" class="text-xs px-2 py-1 bg-white/[0.06] rounded hover:bg-white/[0.1]">Editar</button>
                    <button @click="deleteConnection(conn)" class="text-xs px-2 py-1 bg-red-400/10 text-red-400 rounded hover:bg-red-400/20">Eliminar</button>
                </div>
            </div>
        </div>
