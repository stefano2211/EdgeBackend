...
const sources = ref<ReactiveMCPSource[]>([])
const tools = ref<ReactiveToolConfig[]>([])
const selectedSource = ref<ReactiveMCPSource | null>(null)
const isViewingDetail = ref(false)

const isLoading = ref(true)
const togglingToolId = ref<number | null>(null)

async function toggleTool(tool: ReactiveToolConfig) {
  if (togglingToolId.value !== null) return
  const original = tool.is_enabled
  try {
    togglingToolId.value = tool.id
    tool.is_enabled = !tool.is_enabled
    await reactiveConfigService.toggleTool(tool.id, tool.is_enabled)
    console.log(`[Tools] Tool ${tool.id} toggled to ${tool.is_enabled}`)
  } catch (e) {
    console.error('Failed to toggle tool', e)
    tool.is_enabled = original
  } finally {
...
async function loadData() {
  ...
  try {
    ...
    console.error('Failed to load reactive tools/sources', e)
  } finally {
    isLoading.value = false
  }
}

function selectSource(source: ReactiveMCPSource) {
...
async function createSource() {
  try {
    const created = await reactiveToolService.createSource(newSource.value)
...
async function deleteSource(id: number) {
  if (!confirm('¿Estás seguro de eliminar esta fuente reactiva? Se eliminarán todas sus herramientas.')) return
  try {
    await reactiveToolService.deleteSource(id)
    sources.value = sources.value.filter(s => s.id !== id)
    if (selectedSource.value?.id === id) goBack()
  } catch (e) {
    console.error('Failed to delete reactive source', e)
  }
}

async function deleteTool(id: number) {
  if (!confirm('Are you sure you want to delete this reactive tool?')) return
  try {
    await reactiveToolService.deleteTool(id)
    tools.value = tools.value.filter(t => t.id !== id)
  } catch (e) {
    console.error('Failed to delete reactive tool', e)
  }
}
  ...
  <div class="px-6 py-6 max-w-6xl mx-auto space-y-8 h-full overflow-y-auto custom-scrollbar">
    ...
    <template v-if="!isViewingDetail">
      <div class="flex items-center justify-between mb-8">
        <div>
          <h2 class="text-2xl font-bold text-white tracking-tight">Fuentes MCP reactivas</h2>
          <p class="text-[#7a7a7a] text-sm mt-1">Gestiona APIs reactivas y sus conjuntos de herramientas para el sistema de eventos.</p>
        </div>
        <button 
      ...
      >
        <div v-if="isAddingSource" class="bg-[#2f2f2f]/30 border border-violet-500/20 rounded-3xl p-6 mb-8 backdrop-blur-sm">
          <h3 class="text-lg font-semibold text-white mb-4">Nueva fuente MCP reactiva</h3>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div v-if="!isLoading && sources.length === 0" class="col-span-full py-20 bg-white/[0.02] border border-dashed border-white/10 rounded-3xl text-center">
          <p class="text-[#7a7a7a] mb-4">No hay fuentes reactivas definidas aún.</p>
          <button @click="isAddingSource = true" class="text-violet-400 font-medium hover:text-violet-300 transition-colors">Crea tu primera fuente MCP reactiva &rarr;</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="selectedSource">
      ...
      <div class="max-w-3xl mx-auto space-y-8">
          <section>
            <h3 class="text-[#7a7a7a] text-[11px] font-bold uppercase tracking-widest mb-5 ml-1">Herramientas reactivas configuradas ({{ sourceTools.length }})</h3>
            <div class="grid grid-cols-1 gap-4">
                ...
                <div class="flex items-center gap-4">
                  ...
                  >
                    <div 
                      ...
                      :class="[
                        tool.is_enabled 
                          ? 'left-6 bg-violet-400 shadow-[0_0_8px_rgba(139,92,246,0.5)]' 
                          : 'left-1 bg-[#4a4a4a]'
                      ]"
                    ></div>
                  </div>
                  <button @click.stop="deleteTool(tool.id)" class="p-2 text-[#3a3a3a] hover:text-red-400 transition-colors">
              ...
              <div v-if="sourceTools.length === 0" class="text-center py-10 bg-white/[0.01] border border-dashed border-white/5 rounded-2xl">
                <p class="text-[#4a4a4a] text-sm">No hay herramientas reactivas registradas para esta fuente aún.</p>
              </div>
            </div>
          </section>
