  ...
  <div class="px-6 py-6 max-w-6xl mx-auto space-y-8 h-full overflow-y-auto custom-scrollbar">
    ...
    <template v-if="!isViewingDetail">
      <div class="flex items-center justify-between mb-8">
        <div>
          <h2 class="text-2xl font-bold text-white tracking-tight">Reactive MCP Sources</h2>
          <p class="text-[#7a7a7a] text-sm mt-1">Manage reactive APIs and their curated toolsets for the event system.</p>
        </div>
        <button 
      ...
      >
        <div v-if="isAddingSource" class="bg-[#2f2f2f]/30 border border-violet-500/20 rounded-3xl p-6 mb-8 backdrop-blur-sm">
          <h3 class="text-lg font-semibold text-white mb-4">New Reactive MCP Source</h3>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div v-if="!isLoading && sources.length === 0" class="col-span-full py-20 bg-white/[0.02] border border-dashed border-white/10 rounded-3xl text-center">
          <p class="text-[#7a7a7a] mb-4">No reactive sources defined yet.</p>
          <button @click="isAddingSource = true" class="text-violet-400 font-medium hover:text-violet-300 transition-colors">Create your first Reactive MCP Source &rarr;</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="selectedSource">
      ...
      <div class="max-w-3xl mx-auto space-y-8">
          <section>
            <h3 class="text-[#7a7a7a] text-[11px] font-bold uppercase tracking-widest mb-5 ml-1">Configured Reactive Tools ({{ sourceTools.length }})</h3>
            <div class="grid grid-cols-1 gap-4">
              ...
              <div v-if="sourceTools.length === 0" class="text-center py-10 bg-white/[0.01] border border-dashed border-white/5 rounded-2xl">
                <p class="text-[#4a4a4a] text-sm">No reactive tool registered for this source yet.</p>
              </div>
            </div>
          </section>
